...
# get all the aliases in the system
class GetAliases(APIView):
    def get(self, request, format=None):
        aliases = Aliases.objects.all()
...
class Diff(APIView):
    ...
    def removeAnalogousAliases(self, module1, module2):
        ...
        aliasesForModule1 = {Alias(vendor=Aliases.vendor, device=Aliases.device, subvendor=Aliases.subvendor, subdevice=Aliases.subdevice) for Aliases in Aliases.objects.filter(module=module1)}
        aliasesForModule2 = {Alias(vendor=Aliases.vendor, device=Aliases.device, subvendor=Aliases.subvendor, subdevice=Aliases.subdevice) for Aliases in Aliases.objects.filter(module=module2)}
    ...
    # get the aliases for a Module
    def getAliasesForModule(self, module):
        aliases_to_return = []
        # create a list of namedtuples (aliases), and return the ones that are different for each module version
        Alias = namedtuple('Alias', 'vendor, device, subvendor, subdevice')
        aliases = {Alias(vendor=Aliases.vendor, device=Aliases.device, subvendor=Aliases.subvendor, subdevice=Aliases.subdevice) for Aliases in Aliases.objects.filter(module=module)}
    ...
    def removeAnalogousModules(self, kernelVersion1, kernelVersion2):
        # create the sets of all Modules related to one of the kernel versions
        modsInKernelVersion1 = {
            Module.id for Module in Module.objects.filter(kernelVersions=kernelVersion1)
        }
        modsInKernelVersion2 = {
            Module.id for Module in Module.objects.filter(kernelVersions=kernelVersion2)
        }

        # return the sets of Modules that only show up in one of the kernel versions
    ...
    def sortModulesAndPlaceInModuleCategories(self, kernelVersion1Modules, kernelVersion2Modules):
        ...
        moduleNamesInKernelVersion1 = {
            Module.name for Module in kernelVersion1Modules
        }
        moduleNamesInKernelVersion2 = {
            Module.name for Module in kernelVersion2Modules
        }
    ...
    def getModuleObject(self, module, kernelObject):
        module = Module.objects.get(name=module, kernelVersions=kernelObject)
...
# # get all enabled (and not enabled) kernels (with errata entered) for a particular piece of hardware (via alias)
# class DiffAlias(APIView):
#     # creates the lists of enablered and non-enabled kernels and returns the JSON response
#     def get(self, request, alias, format=None):
#         # the enabled kernel list
#         enabledKernels = []
#         nonEnabledKernels = []
...
#         # send a response with the diff json structure
#         return HttpResponse(json.dumps(aliasLists), content_type="application/json")

# # get all enabled (and not enabled) kernels (with errata entered) for a particular piece of hardware (via device name)
# class DiffDeviceName(APIView):
