...
# KernelVersion is the version of the kernel that contains specific modules
class KernelVersion(models.Model):
    name = models.CharField(max_length=100, unique=True)
    errata = models.CharField(max_length=100, unique=True, null=True, blank=True)
...
class Module(models.Model):
    ...
    srcversion = models.CharField(max_length=24)
    kernelVersions = models.ManyToManyField(KernelVersion)
...
@receiver(m2m_changed, sender=Module.kernelVersions.through)
def verify_uniqueness(sender, **kwargs):
    ...
    if action == 'pre_add':
        for kernelVersion in kernelVersions:
            if Module.objects.filter(name=module.name).filter(kernelVersions=kernelVersion):
                raise IntegrityError('Module with name %s already exists for kernel version %s' % (module.name, KernelVersion.objects.get(pk=kernelVersion)))
...
class Aliases(models.Model):
    ...
    # these values must be hardcorded null in order for the unique_together constraint to Aliases
    class Meta:
